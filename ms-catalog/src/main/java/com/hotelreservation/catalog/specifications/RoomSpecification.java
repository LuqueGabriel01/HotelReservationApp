...
public class RoomSpecification {
  ...
  /**
   * Creates a specification to filter rooms by their category classification type. Maps the
   * incoming string to the {@link RoomType} enum.
   *
   ...
   * @throws InvalidFilterException If the provided type string does not match any valid enum
   *     literal.
   */
  ...
  /**
   * Creates a specification to filter rooms that can accommodate at least a minimum number of
   * guests.
   *
   * @param capacity The required minimum guest capacity. If null, the filter is ignored.
   * @return A Specification for the query checking for a capacity greater than or equal to the
   ...
   * @return A Specification for the query evaluating price boundaries, or null if both parameters
   *     are null.
   */
