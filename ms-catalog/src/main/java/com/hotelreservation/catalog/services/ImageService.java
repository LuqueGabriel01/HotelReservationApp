...
/**
 * Service responsible for managing hotel image assets, handling uploads,
 * deletions, and showcase updates via Cloudinary cloud storage integration.
 */
@Service
...
public class ImageService {
     ...
     * @throws ImageUploadException If an infrastructural error or timeout occurs during the Cloudinary transport layer.
     ...
     * @throws NotFoundException If the image record does not exist or does not belong to the given hotel.
    ...
    public void deleteImage(UUID hotelId, UUID imageId){
        ...
        ImageHotel imageHotel = imageHotelRepository.findById(imageId)
                .orElseThrow(() -> new NotFoundException("Image with id " + imageId + " not found"));

        if (!imageHotel.getHotel().getId().equals(hotelId)){
            throw new NotFoundException("Image with id " + imageId + " not found in hotel with id " + hotelId);
        }
    ...
    /**
     * Designates a specific image as the primary showcase graphic for a hotel.
     * Automatically unmarks any pre-existing main image assigned to that hotel beforehand.
     *
     * @param hotelId The unique identifier of the parent hotel.
     * @param imageId The unique identifier of the image to promote.
     * @return The updated and mapped image response profile.
     * @throws NotFoundException If the hotel or image do not exist, or if the target image is not linked to the hotel.
     */
    @Transactional
    public ImageHotelResponseDto setMainImage(UUID hotelId, UUID imageId){
        ...
        imageHotelRepository.findMainImageByHotelId(hotelId)
                .ifPresent(current -> {
                    current.unmarkAsMain();
                    imageHotelRepository.save(current);
                });

        ImageHotel newMain = imageHotelRepository.findById(imageId)
                .orElseThrow(() -> new NotFoundException("Image with id " + imageId + " not found"));
