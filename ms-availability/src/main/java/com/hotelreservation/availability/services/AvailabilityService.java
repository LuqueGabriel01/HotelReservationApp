...
public class AvailabilityService {
     ...
     * <p>
     * This method removes the temporary block status for the given dates, persists
     * the new reservation status linked to the booking, and deletes the block record.
     *
     ...
     */
    @Transactional
    public ReserveRoomResponseDto reserveBooking(ReserveRoomRequestDto reserveRoomRequestDto, UUID roomId){

        RoomBlock roomBlock = blockRepository.findById(reserveRoomRequestDto.lockId())
                .orElseThrow(() -> new NotFoundException(ErrorConstants.Error.ROOM_BLOCK_WITH_ID + reserveRoomRequestDto.lockId() + ErrorConstants.Error.NOT_FOUND_SUFFIX));
        ...
        List<RoomAvailability> reservedDates = dates.stream()
        ...
        return new ReserveRoomResponseDto(roomId, reserveRoomRequestDto.bookingId(), AvailabilityStatus.RESERVED);
    }
    ...
    public BlockReleaseResponseDto releaseBooking(ReleaseRoomRequestDto requestDto, UUID roomId){
        ...
        List<RoomAvailability> bookedRooms = availabilityRepository.findByBookingId(requestDto.bookingId()).stream().toList();

        if (bookedRooms.isEmpty()){
            throw new NotFoundException(ErrorConstants.Error.BOOKING_WITH_ID + requestDto.bookingId() + ErrorConstants.Error.NOT_FOUND_SUFFIX);
        }
