...
import com.hotelreservation.availability.exception.ForbiddenException;
...
public class BlockService {
  ...
  /**
   * Blocks a specific hotel room for a user within a given date range. * @param hotelId the unique
   * identifier of the hotel
   *
   ...
   * @throws InvalidDateRangeException if the date range is invalid (same day or checkout before
   *     check-in)
   ...
   * @throws RoomNotAvailableException if the room is already occupied or blocked for the requested
   *     dates
   */
  @Transactional
  public RoomBlockResponseDto blockRoom(
      UUID hotelId, UUID roomId, UUID userId, LocalDate checkIn, LocalDate checkOut) {
    ...
    List<RoomAvailability> blockedDates =
        checkIn
            .datesUntil(checkOut)
            .map(
                date ->
   ...
   *
   * <p>This operation also releases the associated blocked dates from the availability record.
   *
