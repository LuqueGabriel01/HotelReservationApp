...
public class StaleBlockCleanupJob {
    ...
    /**
     * Periodically removes expired room blocks and releases their associated
     * availability dates back to the inventory.
     * <p>
     * Runs automatically at a fixed rate of every 60 seconds.
     */
    @Scheduled(fixedRate = 60000)
    @Transactional
    public void cleanupExpiredBlock(){
        List<RoomBlock> expiredBlocks = blockRepository.findByExpiresAtBefore(Timestamp.from(Instant.now()));
        ...
        for (RoomBlock block : expiredBlocks){
            List<LocalDate> dates = block.getCheckIn()
                    .datesUntil(block.getCheckOut())
                    .toList();
