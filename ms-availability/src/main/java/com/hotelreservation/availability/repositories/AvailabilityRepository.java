...
public interface AvailabilityRepository extends JpaRepository<RoomAvailability, UUID> {
  ...
  Set<UUID> findOccupiedRoomIds(
      @Param("roomIds") List<UUID> roomIds, @Param("dates") List<LocalDate> dates);
   ...
   */
  List<RoomAvailability> findByRoomIdAndDateIn(UUID roomId, List<LocalDate> dates);
  ...
  @Modifying
  @Query("DELETE FROM RoomAvailability a WHERE a.roomId = :roomId AND a.date IN :dates")
  void deleteByRoomIdAndDateIn(UUID roomId, List<LocalDate> dates);

  List<RoomAvailability> findByBookingId(UUID booking);
}
