 ...
 * <p>
 * This class handles the creation, validation, and parsing of JWT tokens used
 * for user authentication and authorization within the application.
 * </p>
 */
@Service
...
public class JwtService {
    ...
    /**
     * Generates a long-lived Refresh Token used to request new access tokens without re-authenticating.
     *
     * @param user the user entity for whom the token is being generated
     ...
     * <p>
     * If the token signature is invalid, expired, or malformed, the cryptographic exception
     * is caught internally and safely returns null.
     * </p>
     *
     ...
     * <p>
     * If the token parsing fails due to bad structural configurations or signature mismatch,
     ...
     * <p>
     * A token is considered authentic and valid if its extracted username matches the
     * database record's email, its cryptographic signature is intact, and it has not expired.
     * </p>
     *
     ...
     *
     * @param token the raw JWT token string
     * @return true if the token has expired or is invalid; false if it is still within its validity window
     */
    ...
    /**
     * Centralized builder to assemble custom claims, structural headers, and signatures into a JWT string.
     *
