 ...
 *
 * <p>This class configures stateless session management, wires custom authentication mechanisms,
 * sets up endpoint authorization rules, and hooks up explicit handlers for authorization failures
 * and sign-out logic.
 */
@Configuration
...
public class SecurityConfig {
  ...
  /**
   * Configures the {@link SecurityFilterChain} to define HTTP security policies, public/private
   * paths, custom filters, and error endpoints.
   *
        ...
        .authorizeHttpRequests(
            auth ->
                auth.requestMatchers(HttpMethod.GET, ApiPaths.Actuator.HEALTH)
                    ...
                    .permitAll()
                    .requestMatchers(HttpMethod.POST, ApiPaths.Auth.REFRESH)
                    .permitAll()
                    .requestMatchers(HttpMethod.POST, ApiPaths.Auth.LOGIN)
                    .permitAll()
                    .requestMatchers(HttpMethod.GET, ApiPaths.Auth.ME)
                    .authenticated()
                    .requestMatchers(HttpMethod.PUT, ApiPaths.Auth.ME)
                    .authenticated()
                    .anyRequest()
                    .authenticated())
        .sessionManagement(
